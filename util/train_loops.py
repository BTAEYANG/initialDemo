...
def train_distill(epoch, train_loader, module_list, criterion_list, optimizer, opt, new_beta):
    ...
    for idx, data in enumerate(train_loader):
        ...
        if opt.distill == 'kd':
            ...
            t_tensor, s_tensor, t_fc_tensor, s_fc_tensor = module_list[1](feat_t[:-1], feat_s[:-1], embed_s, embed_t, model_t)
            loss_kd = criterion_kd(t_tensor, s_tensor, t_fc_tensor, s_fc_tensor)
        else:
            raise NotImplementedError(opt.distill)

        loss = opt.gamma * loss_cls + opt.alpha * loss_kl + opt.new_beta * loss_kd
